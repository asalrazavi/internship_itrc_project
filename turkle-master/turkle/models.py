...
class Proficiency(models.Model):
    """For storing all the proficiencies"""
...
class UserProficiency(models.Model):
    """For assigning the proficincies to the users"""
...
class PhoneNumber(models.Model):
    ...
    def has_main_phone_number(self):
        pass
...
class BankAccountInformation(models.Model):
    ...
    shaba_number = models.CharField(max_length=26, blank=True, null=True)
...
class UserEvaluation(models.Model):
    ...
    def deactivate_user_with_low_rating(self, user):
        ...
        minimum_calibration_count = project.minimum_calibration_count
        calibraion_rating = project.calibration_threshold

        if self.count >= minimum_calibration_count:
...
class TaskAssignment(models.Model):
    ...
    @classmethod
    def get_task_assignments(cls, project=None, batch=None):
        queryset = cls.objects.all()

        if project:
            queryset = queryset.filter(task__batch__project=project)
        if batch:
            queryset = queryset.filter(task__batch=batch)
    ...
    def check_majority_voting(done_taskassignments, task_id):
        ...
        minimum = batch.minimum_same_answers
...
class Batch(TaskAssignmentStatistics, models.Model):
    ...
    minimum_same_answers = models.IntegerField(null=True, blank=True)
    evaluation_rate = models.FloatField(validators=[MinValueValidator(0.0), MaxValueValidator(1.0)], default=0)
    ...
    def create_tasks_from_csv(self, csv_fh):
        ...
        for row in data_rows:
            ...
            task.save()
            num_created_tasks += 1
        self.calculate_evaluation_rate(num_created_tasks)
        logger.info('Created %i tasks for Batch(%i) %s', num_created_tasks, self.id, self.name)

        return num_created_tasks

    def calculate_evaluation_rate(self, task_num):
        input_csv = Task.objects.filter(batch=self).first().input_csv_fields
        if "calibration_answer" in input_csv:

            num_calibration_tasks = Task.objects.filter(batch=self)\
                .exclude(input_csv_fields__contains='"calibration_answer": "0"').count()

            # calculate the rate of calibraton tasks:
            # the tare is calculating the 2/3 of the calibration tasks on the none calibration tasks

            num_calib = (num_calibration_tasks * 2) / 3
            num_none_calib = task_num - num_calibration_tasks
            rate = num_calib / num_none_calib
...
class ProjectAnalyticsInformation(models.Model):
    PROJECT_TYPE_CHOICES = [(0, 'close task'),
                            (1, 'open-ended task'),
                            (2, 'both')]

    INPUT_TYPE_CHOICES = [(1, 'Single choice'),
                          (2, 'Multiple choice')]

    project = models.ForeignKey('Project', verbose_name='project', on_delete=models.CASCADE)
    name = models.CharField(max_length=100, blank=True)
    project_type = models.IntegerField(choices=PROJECT_TYPE_CHOICES, default=0)
    input_type = models.IntegerField(choices=INPUT_TYPE_CHOICES)
    input_names_list = JSONField()


    def one_column_distribution(self, batch=None, project=None):
        if batch:
            queryset = TaskAssignment.get_task_assignments(batch=batch)
        if project:
            queryset = TaskAssignment.get_task_assignments(project=project)

        result = list(queryset.values_list('answers', flat=True))

        distribution = dict(Counter(item[self.name] for item in result if item and self.name in item.keys()))

        return distribution

    def multi_column_distribution(self, batch=None, project=None):
        if batch:
            queryset = TaskAssignment.get_task_assignments(batch=batch)
        if project:
            queryset = TaskAssignment.get_task_assignments(project=project)
        ...
        result = list(queryset.values_list('answers', flat=True))
        ans_list = [i for i in self.input_names_list for item in result if item != '' and i in item.keys()]
        distribution = dict(Counter(ans_list))
...
class Project(TaskAssignmentStatistics, models.Model):
    ...
    calibration_threshold = models.FloatField(validators=[MinValueValidator(0.0), MaxValueValidator(1.0)], default=0)
    cost_rate = models.FloatField(validators=[MinValueValidator(0.0)], default=0)
...
class Payment(models.Model):
    ...
    paymented_at = models.DateTimeField(null=True, blank=True)

    def calculate_salary(self):
        # salary = Batch.total_assignments_completed_by(self.batch_id, self.user_id) * self.project_id.cost_rate
        salary = self.task_count * self.batch.project.cost_rate
        return salary

    @staticmethod
    def payment_user_in_the_batch(user, batch):

        if Payment.objects.filter(batch=batch, user=user):

            # user has previous records of payment for the batch
            p = Payment.objects.filter(batch=batch, user=user).last()
            if not p.payment_status:
                p.task_count += 1
                p.salary = p.calculate_salary()
                p.save()
            elif p.payment_status:
                salary = batch.project.cost_rate
                p = Payment(user=user, project=batch.project, batch=batch,
                            task_count=1, salary=salary)
                p.save()
