...
from django.forms import (FileField, FileInput, HiddenInput, IntegerField, Media, NumberInput,
...
from .models import ActiveUser, ActiveProject, Batch, Project, TaskAssignment, UserEvaluation, ProjectManager, \
                    BatchManager, Payment, PhoneNumber, Proficiency, ProjectAnalyticsInformation
...
class CustomUserAdmin(UserAdmin, AjaxAutocompleteListFilterModelAdmin):
    ...
    list_display = ('username', 'email', 'first_name', 'last_name', 'is_staff', 'is_active',
                    'get_phone', 'stats', 'profile')
    autocomplete_list_filter = ('groups',)

    @admin.display(description='Phone number')
    def get_phone(self, obj):
        phone = PhoneNumber.objects.filter(user=obj, verified_at__isnull=False).last()
        if phone:
    ...
    def profile(self, obj):
        stats_url = reverse('profile', kwargs={'username': obj.username})
        return format_html('<a href="{}" class="button">View Profile</a>'.
                           format(stats_url))
...
class BatchForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['minimum_same_answers'].label = 'Minimum Users'
        self.fields['minimum_same_answers'].help_text = 'The minimum users needed for majority voting'
        self.fields['minimum_same_answers'].required = False
    ...
    def clean(self):
        ...
        is_majority_voting = cleaned_data.get("is_majority_voting")
        minimum_same_answers = cleaned_data.get("minimum_same_answers")

        if is_majority_voting is True and not minimum_same_answers:
            raise ValidationError("Minimum Users can not be empty when Majority voting is checked")
...
class BatchAdmin(AjaxAutocompleteListFilterModelAdmin):
    ...
    list_display = (
        'name', 'project', 'is_active', 'assignments_completed', 'tasks_completed',
        'stats', 'charts', 'download_input', 'download_csv', 'minimum_same_answers', 'evaluation_rate'
        )
    ...
    def tasks_completed(self, obj):
        ...
        h = format_html(
            '<progress value="{0}" max="{1}" title="Completed {0}/{1} Task Assignments">'
            '</progress> '.format(tft, ta))
        if tft >= ta:
            h += _boolean_icon(True)
        else:
            h += format_html('<img src="{}" />', static('admin/img/icon-unknown-alt.svg'))
        h += format_html(' {} / {}'.format(tft, ta))
        return h
    ...
    def charts(self, obj):
        stats_url = reverse('charts_batch', kwargs={'batch_id': obj.id})
        return format_html('<a href="{}" class="button">View Charts</a>'.
                           format(stats_url))
        ...
        if not obj:
            ...
            return (
                ...
                ('Majority voting Settings', {
                    'fields': ('is_majority_voting', 'minimum_same_answers')
                }),
                ('Permissions', {
                    'fields': (
...
class ProjectAnalyticsInformationInline(admin.StackedInline):
    model = ProjectAnalyticsInformation
    extra = 1
...
class ProjectForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['minimum_calibration_count'].help_text = 'the minimum tasks to check the user\'s rating'
        self.fields['minimum_calibration_count'].required = False
        self.fields['calibration_threshold'].help_text = 'the minimum rating for calibration tasks'
        self.fields['calibration_threshold'].widget = NumberInput(attrs={'step': '0.1'})
        self.fields['calibration_threshold'].required = False
        ...
        self.fields['active'].help_text = 'Deactivating a Project effectively deactivates ' + \
            ...
            'itself and the associated Project are Active.'
        self.fields['cost_rate'].help_text = 'the cost rate for every task in this project in RIAL.'
    ...
    def clean(self):
        ...
        evaluation_script = cleaned_data.get("evaluation_script")
        minimum_calibration_count = cleaned_data.get("minimum_calibration_count")
        calibration_threshold = cleaned_data.get("calibration_threshold")
        cost_rate = cleaned_data.get("cost_rate")

        if evaluation_script and not minimum_calibration_count:
            raise ValidationError("Minimum Calibration Count can not be empty when there is Evaluation Script")
        if evaluation_script and (not calibration_threshold or calibration_threshold == 0):
            raise ValidationError("Calibration Threshold can not be empty or 0 when there is Evaluation Script")
...
class ProjectAdmin(GuardedModelAdmin, AjaxAutocompleteListFilterModelAdmin):
    ...
    inlines = [ProjectAnalyticsInformationInline]
    ...
    def charts(self, obj):
        stats_url = reverse('charts_project', kwargs={'project_id': obj.id})
        return format_html('<a href="{}" class="button">View Charts</a>'.
                           format(stats_url))
    ...
    def get_fieldsets(self, request, obj=None):
        if not obj:
            ...
            return (
                ...
                ('Evaluation', {
                    'fields': ('evaluation_script',
                               'minimum_calibration_count', 'calibration_threshold')
                }),
                ('Salary', {
                    'fields': ('cost_rate',)
                }),
    ...
    def get_list_display(self, request):
        if request.user.has_perm('turkle.add_batch'):
            return ('name', 'filename', 'updated_at', 'active', 'stats', 'charts', 'publish_tasks',
                    'minimum_calibration_count', 'calibration_threshold', 'evaluation_script')
        else:
            return ('name', 'filename', 'updated_at', 'active', 'stats', 'charts',
                    'minimum_calibration_count', 'calibration_threshold', 'evaluation_script')
...
@admin.action(description="Mark selected row as paid")
def paid_payment(modeladmin, request, queryset):

    updated = queryset.update(payment_status=True, paymented_at=timezone.now())
    modeladmin.message_user(
        request,
        ngettext(
            "%d  was successfully marked as paymented.",
...
class PaymentAdmin(AjaxAutocompleteListFilterModelAdmin):

    list_display = ('id', 'user', 'project', 'batch', 'task_count', 'salary', 'payment_status', 'paymented_at', 'project_salary')
    readonly_fields = ('user', 'project', 'batch', 'salary', 'task_count')
    list_per_page = 20
    preserve_filters = False
    list_filter = ['payment_status']
    autocomplete_list_filter = ['batch', 'user', 'project']

    actions = [paid_payment]

    def get_queryset(self, request):
        projects = ProjectManager.get_queryset(self, request)
        # query = Payment.objects.filter(project_id__in=projects).values('project_id').annotate(no_batch=Count('batch_id'), no_project=Count('project_id'), salary_sum=Avg('salary'))
        query = Payment.objects.filter(project__in=projects)

        return query


    def project_salary(self, obj: Payment):
        total_sum = Payment.objects.filter(project=obj.project, user=obj.user).aggregate(Sum('salary'))
        return total_sum["salary__sum"]


class ProficiencyAdmin(GuardedModelAdmin, AjaxAutocompleteListFilterModelAdmin):
    search_fields = ['name']
